...
async def admin_setup(
    ...
    config_data = {
        ...
        "ticket_category_id": str(support_category.id), # NEW
        ...
        "app_log_channel_id": str(app_logs.id), # NEW
        "staff_role_id": str(staff_role.id), # NEW
        "ticket_support_role_id": str(support_role.id), # NEW
...
@bot.tree.command(name="pair", description="Cupid: Manually pair")
@app_commands.default_permissions(manage_messages=True)
async def manual_pair(interaction: discord.Interaction, user1: discord.Member, user2: discord.Member):
    ...
    await interaction.response.send_message(f"🏹 Paired {user1.mention} and {user2.mention}!")
